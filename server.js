...
const FACE_DB_PATH = path.join(ROOT_DIR, "face_db.json");
const FACE_DB_CLOUD_ID = "system_face_id_backup.enc";
...
function getBackupKeyMaterial() {
  return (
    ...
    "change-me-face-backup-key"
  );
}

function encryptJsonObject(data) {
  const secret = getBackupKeyMaterial();
...
function decryptJsonObject(raw) {
...
async function backupFaceDBToCloud() {
  ...
  try {
    const raw = fs.readFileSync(FACE_DB_PATH, "utf8");
    const encrypted = encryptJsonObject(JSON.parse(raw || "[]"));
    const tmpPath = path.join(ROOT_DIR, "face_db.encrypted.tmp");
    fs.writeFileSync(tmpPath, encrypted, "utf8");
    await withCloudinaryAccount(0, async () => {
      await cloudinaryLib.uploader.upload(tmpPath, {
...
async function restoreFaceDBFromCloud() {
  ...
  try {
    const url = await withCloudinaryAccount(0, async () => {
      return cloudinaryLib.url(`system_backup/${FACE_DB_CLOUD_ID}`, {
        resource_type: "raw",
        secure: true,
      });
    });
    ...
    if (response.status < 200 || response.status >= 300 || !response.data) {
      throw new Error("File backup chưa tồn tại hoặc lỗi mạng");
    }

    const data = decryptJsonObject(response.data);
    fs.writeFileSync(FACE_DB_PATH, JSON.stringify(data, null, 2), "utf8");
    console.log(">> [SYSTEM] 👌 Khôi phục dữ liệu Face ID thành công!");
  } catch (error) {
    console.log(
      `>> [SYSTEM] ⚠️ Chưa có bản backup hoặc lỗi (${error.message}). ❌ Tạo Database rỗng.`
    );
    if (!fs.existsSync(FACE_DB_PATH)) {
      fs.writeFileSync(FACE_DB_PATH, "[]", "utf8");
    }
  }
}
...
async function readStoredFaceData() {
  if (mongoose.connection?.readyState === 1) {
    ...
    if (doc?.descriptors?.length) {
      return [{ label: doc.label, descriptors: doc.descriptors }];
    }
  }
...
async function writeStoredFaceData(label, descriptors) {
  if (mongoose.connection?.readyState === 1) {
  ...
  if (fs.existsSync(FACE_DB_PATH)) {
    try {
      users = JSON.parse(fs.readFileSync(FACE_DB_PATH, "utf8"));
    } catch (_) {
      users = [];
    }
  }
  users = Array.isArray(users) ? users.filter((u) => u.label !== label) : [];
  users.push({ label, descriptors });
  fs.writeFileSync(FACE_DB_PATH, JSON.stringify(users, null, 2), "utf8");
  await backupFaceDBToCloud();
  return { mode: "file" };
}
...
app.post("/face-id/register", requireSameOrigin, requireAdmin, async (req, res) => {
  try {
    ...
    return res.json({
      ...
      message:
        result.mode === "mongo"
          ? "Đã lưu FaceID vào MongoDB"
          : "Mongo chưa sẵn sàng, đã lưu tạm vào file.",
    });
  } catch (e) {
...
app.delete(
  ...
  async (req, res) => {
    try {
      ...
      return res.json({ success: true, message: "Đã xóa dữ liệu FaceID" });
    } catch (e) {
